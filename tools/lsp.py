...
xml_template_final = """<?xml version="1.0" encoding="UTF-8"?>
<zabbix_export>
    <version>2.0</version>
...
if __name__ == "__main__":
    ...
    for name, oid in lsps.iteritems():
        items += xml_template_items.format(name = name, oid = oid)
        graphs += xml_template_graph.format(name = name, oid = oid)

    final_xml = xml_template_final.format(items = items, graphs = graphs)

    with open('lsps.xml','wr') as f:
        f.write(final_xml)
        print 'Saved: lsps.xml'
